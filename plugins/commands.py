...
from database.apisdb import add_sundisk, get_sundisk
        ...
        buttons = [
            [
                InlineKeyboardButton(
                    '🤖 Developer', url='https://t.me/cyniteofficial')
            ],
            [
                InlineKeyboardButton(
...
async def api_adder(bot, message):
    sts = await message.reply("Checking Your Request...")
    if " " not in message.text:
        return await message.reply("Use correct format.<code>/addapi (group id) (api)</code>\n\n\nExample <code>/addapi -100123456  xxxxxxxxxxxxxxxxxxxxx</code>.")
    data = message.text.strip().split(" ")
    try:
        cmd, chat_id, api = data
        await add_sundisk(chat_id, api)
